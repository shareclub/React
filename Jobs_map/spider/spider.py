...
newTable="C:\\Users\\Administrator\\Desktop\\projects\\code\\project\\spider\\"+fileName+".xlsx"#表格名称
wb = xlwt.Workbook(encoding='utf-8')#创建excel文件，声明编码
ws = wb.add_sheet('sheet1')#创建表格
headData = ['name','company','address','salary','data']#表头部信息
for colnum in range(0, 5):
    ws.write(0, colnum, headData[colnum], xlwt.easyxf('font: bold on'))  # 行，列


for each in range(1,page):
    index=(each-1)*50+1
